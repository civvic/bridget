...
var ObserverManager = class {
  ...
  register(name, { target, options, callback }, autostart = false, append = false) {
    if (this.has(name)) {
      ...
      const callbacks = /* @__PURE__ */ new Set([callback]);
  ...
  isEmpty(name) {
    return !this.has(name) || this.observers.get(name).callbacks.size === 0;
  }
  ...
  stop(name, { removeTarget = false } = {}) {
    ...
    if (removeTarget) config.target = null;
...
var observer_default = getObserverManager;
export {
